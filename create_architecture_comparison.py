"""
Create architecture comparison table visualization for all three experiments.
"""
...
# Title
fig.suptitle('Architecture Comparison: Three Experimental Approaches',
...
# Create table
n_rows = len(comparison_data['Aspect'])
n_cols = 4  # Aspect + 3 experiments
...
# Add legend
legend_y = start_y - (n_rows + 1) * cell_height - 0.03
fig.text(
    start_x, legend_y,
...
fig.suptitle('Architecture Flow Diagrams', fontsize=18, fontweight='bold', y=0.98)

experiments = [
    {
        'title': 'E1: OPG-only Baseline',
        'components': [
            'OPG Images',
            '↓',
            'MedGemma-4B\nVision Encoder\n(Frozen)',
        ...
        'colors': ['lightblue', 'white', 'lightcoral', 'white', 'lightyellow', 'white', 'lightgreen', 'white', 'orange']
    },
    {
        'title': 'E2: Late Fusion',
        'components': [
            'Intraoral    OPG    Ceph',
            '↓            ↓      ↓',
            'CNN₁   CNN₂   CNN₃\n(Trainable)',
        ...
        'colors': ['lightblue', 'white', 'lightgreen', 'white', 'lightyellow', 'white', 'lightcoral', 'white', 'orange']
    },
    {
        'title': 'E3: Multi-Image Prompting',
        'components': [
            'Multi-Modal Images\n+ Prompts',
            '↓',
            'MedGemma-4B\nVision+Text Encoder\n(Frozen)',
...
for idx, (ax, exp) in enumerate(zip(axes, experiments)):
    ax.set_xlim(0, 10)
    ax.set_ylim(0, len(exp['components']))
    ax.axis('off')
    ax.set_title(exp['title'], fontsize=14, fontweight='bold', pad=20)

    for i, (comp, color) in enumerate(zip(exp['components'], exp['colors'])):
        y_pos = len(exp['components']) - i - 0.5
        ...
        if '↓' in comp:
            ...
            ax.text(5, y_pos, comp, ha='center', va='center',
                   fontsize=10, fontweight='bold')

plt.tight_layout()
plt.savefig('visualizations/architecture_flow_diagrams.png',
            dpi=300, bbox_inches='tight', facecolor='white')
print("✓ Saved: visualizations/architecture_flow_diagrams.png")

print("\n✓ Architecture comparison visualizations created successfully!")
